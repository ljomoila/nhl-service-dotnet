...
public class NhlClientTest
{
    ...
    public async void TestGetTeams_Success()
    {
        ...
        HttpClient client = CreateHttpClient(response, HttpStatusCode.OK);

        // Act
        Team[]? teams = await new NhlClient(client).GetTeams();
    ...
    public void TestGetTeams_InvalidStatus()
    {
        ...
        HttpClient client = CreateHttpClient("{}", expectedStatus);

        // Act
        var ex = Assert.ThrowsAsync<NhlException>(
            async () => await new NhlClient(client).GetTeams()
        );
